...
def r_squared_plot( ax, solution, prediction, error_bounds=[1,2,5,10], box_loc=None, rsquare_number=True, **axset ):
    ...
    NOTE that this function does not nicely work for values > 100
    Parameters:
    -----------
    ...
    for current_color, error in reversed( list( zip( error_colors[:n_errors], np.sort( error_bounds) ))) :
        ...
        ax.scatter( *2*[-100], color=current_color, **base_kwargs, s=20, label=label)
    ...
    if rsquare_number is True:
        ax.text( 0.48, 0.92, r'$R^2\approx {:1.4f}$'.format( rsquare),  transform=ax.transAxes )
    if xmin < 0:
    ...
    ax.set( **axset)
    plot.add_legend( ax, 'top left')
    return ax



def binned_errors( x, y, y_pred, n_bins=15, line_threshold=None, error_measure='component wise', quantiles=[0.99, 0.95, 0.90], extra_legend='figures/bin_error_legend.pdf'):
    """
    ...
    reference_style = dict(s=15, color=uniS.lightblue, edgecolor='black')
    prediction_style = dict(s=5, linewidth=0.02, color=uniS.red, edgecolor='black')#slightly smaller than true values
    ...
    n_bot = 1
    ylabels = [r'$\bar\kappa_{11} [-]$', r'$\bar\kappa_{22} [-]$', r'$\sqrt2\bar\kappa_{12} [-]$' ]
    for i in range( n_target):
